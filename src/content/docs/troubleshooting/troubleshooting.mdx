...
If failures continue after URL/surface fixes, configure `BOTASAURUS_SCRAPER_URL` or use `--strategy botasaurus` for browser rendering.
...
- **`UnsupportedResponseContentType`:** The website returned content that html2rss can't parse (not HTML or JSON). The default HTTP strategy still treats bodies that sniff as HTML (`<!DOCTYPE html` / `<html`) as HTML, and decodes gzip/brotli even when `Content-Encoding` is missing.
- **`UnsupportedStrategy`:** The specified strategy is not available. Use `auto`, `default`, `botasaurus`, or `local_file` (`faraday` and `httpx` are accepted as legacy aliases).
...
- **Dynamic Content:** `default` does not render JavaScript. If content loads dynamically, configure `BOTASAURUS_SCRAPER_URL` or run with `--strategy botasaurus` so the page can be rendered before extraction.
