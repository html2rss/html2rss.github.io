...
Do not expect Faraday-only fetches to succeed against active DataDome interstitials.
...
- **Dynamic Content:** `faraday` does not render JavaScript. If content loads dynamically, configure `BOTASAURUS_SCRAPER_URL` or run with `--strategy botasaurus` so the page can be rendered before extraction.
