...
**Articles now:** `scrape` (or `batch_scrape`). `strategy: "auto"` already falls back to Botasaurus when configured — do not retry with explicit `default` after `auto`. Empty items can still be success; follow `next_step`.
