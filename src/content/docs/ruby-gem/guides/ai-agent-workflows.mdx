...
- Responses are structured (JSON Feed items, YAML configs, RSS XML).
...
2. An empty item list is still success for articles-now. Read `html2rss://runtime` if `botasaurus_configured` is false.
...
1. **Capture:** Call `capture_config`. It returns YAML (`items` + `enhance: true`). Check `_meta.articles_count` and `has_selectors`.
...
5. **Apply:** Call `apply_config`. `isError` plus `_meta.item_count: 0` means the config is not shippable — channel title in the XML is not success.
