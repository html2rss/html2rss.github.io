...
1. Call `scrape_url` with `strategy: "auto"`. Auto runs Faraday then Botasaurus when `BOTASAURUS_SCRAPER_URL` is set on the MCP process, promotes native RSS/Atom when present, and may rewrite a weak homepage URL via entry resolution.
...
3. Call `inspect_url` when the result is weak or you need recon (`final_url`, `status`, `scheme_downgrade`, `alternate_feeds`, `surface_category`). If inspect reports alternate feeds, follow `next_step: scrape_url` — do not treat alternates as `done`.
...
2. **Recon (optional):** Call `inspect_url` for `final_url`, `status`, `scheme_downgrade`, `alternate_feeds`, and `surface_category` if the draft is weak. Alternates mean call `scrape_url` next (native promotion), not stop.
