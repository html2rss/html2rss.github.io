...
5. **Assemble** — `{ items: { selector:, enhance: true } }` plus channel. Capture records the selected strategy when the draft needs to reproduce that fetch behavior during `html2rss apply` / `Html2rss.feed`.
...
- [AI agent workflows (MCP `capture`)](/ruby-gem/guides/ai-agent-workflows/)
