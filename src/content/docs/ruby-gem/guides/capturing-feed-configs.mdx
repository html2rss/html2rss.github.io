...
MCP `capture_config` returns the same YAML. `validate_config` / `apply_config` accept that YAML string (XOR a config hash). `apply_config` is `isError` when the feed has zero items.
