...
description: "Learn how to manage feed configurations with YAML files. Organize, version control, and maintain your html2rss apply configurations effectively."
---
...
- `entry_url` / `scrape` (pasted vs effective URL after entry resolution)
- `entry_resolution` (`applied`, `probe_count`, `reason`, optional `winner_score`)

For URL-only auto discovery, `Html2rss.auto_feed_result(url)` returns the same `FeedResult` (CLI `html2rss scrape --explain` prints `status.to_h` on stderr).
