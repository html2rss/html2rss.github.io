...
`scrape` and `capture` with `strategy: "auto"` run default (HTTPX) → Botasaurus AutoFallback in one call. Prefer native RSS/Atom when present; weak homepage URLs may be rewritten via entry resolution.

`inspect` with `auto` stays on the default HTTP strategy. Pin `strategy: "botasaurus"` when inspect needs browser rendering.
...
| `html2rss://strategies` | `auto`, `default`, `botasaurus` (not `local_file`)                                                                 |
