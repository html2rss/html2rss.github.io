...
`scrape_url` and `capture_config` with `strategy: "auto"` (the default) run Faraday → Botasaurus AutoFallback in one call. That path also promotes native RSS/Atom when present and may run entry URL resolution on weak homepage/hub extracts (see [Auto Source](/ruby-gem/reference/auto-source/) and [Strategy](/ruby-gem/reference/strategy/#auto-default)).

`inspect_url` with `auto` uses Faraday for a cheap diagnostic. Pin `strategy: "botasaurus"` when you need browser rendering for inspect. Inspect reports head `rel=alternate` feeds only — it does not path-guess `/feed`.
...
- **Return value:** Envelope whose `payload` is the diagnostic object:
  ...
  - `surface_category`: page surface class (for example listing vs weak/blocked surfaces).
  - `content_type` / `html_response`.
  - `sst`: nested `{ node_count, degraded, segment_stats }` when SST ran; `segment_stats` is also emitted at the top level when present.
...
- **next_step:** `scrape_url` when head alternate feeds exist (runtime scrape consumes native alternates) or when no articles were found; `capture_config` when articles were found without alternates.
