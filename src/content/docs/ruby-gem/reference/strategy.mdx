...
Within each concrete strategy attempt that uses `auto_source` (no custom `selectors`), the extract path can:

1. Promote native RSS/Atom via the tier-0 `native_feed` scraper (or parse a direct feed URL).
2. Run **entry URL resolution** on a weak homepage/hub extract — probe same-origin listing/feed candidates and sticky-rewrite the scrape URL only when the retry yields items — **before** escalating to Botasaurus.

See [Auto Source](/ruby-gem/reference/auto-source/#entry-url-resolution) for gates, defaults (`entry_resolution.enabled: true`, `max_probes: 5`), and Status fields.

Auto fallback shares one request budget across all strategy attempts. The baseline reserves slots for NativeFeed follow-up and entry-resolution probes (about **+7** requests when both defaults are on). For pagination-heavy or dynamic pages, increase `request.max_requests` (or `--max-requests`) when retries exhaust the budget.
...
When the scrape API reports a timed-out stage, `Html2rss::RequestService::RequestTimedOut` may carry `timeout_phase` as `queue`, `boot`, or `work` (also reflected in the exception message as `timeout_phase=<stage>`). Transport-hop timeouts leave `timeout_phase` unset.
