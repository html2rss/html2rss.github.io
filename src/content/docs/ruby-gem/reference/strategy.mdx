---
title: Strategy
description: "Learn how html2rss chooses request strategies by default with auto fallback, and when to override with default, botasaurus, or local_file."
---
...
- **`auto`** (default): Tries concrete strategies in order: `default` -> `botasaurus`.
- **`default`**: Makes a direct HTTP request (powered by HTTPX). Fast and non-blocking with native HTTP/2, connection pooling, and SSRF protection. Does not execute JavaScript.
...
`auto` falls back to `botasaurus` when `default` errors or extracts zero items. Pin a concrete strategy when you need a specific transport (for example, forcing `botasaurus` directly).
...
`default` is the standard static HTTP transport, powered by HTTPX. When no custom `User-Agent` header is supplied in `headers`, `html2rss` sends `html2rss/<VERSION>` by default.

It features:

- **HTTP/2 & HTTP/1.1**: Automatic ALPN negotiation and multiplexed connection pooling.
- **SSRF protection**: Native private-network IP filtering and redirect validation.
- **Resilient decoding**: HTML is accepted when `Content-Type` includes `text/html`, or when the body sniffs as HTML (`<!DOCTYPE html` / `<html`) and the response is not JSON. Gzip, deflate, and brotli streams are decoded automatically.

:::note
Legacy names `faraday` and `httpx` are accepted as aliases for `default` and emit a deprecation warning.
:::
...
- pin `strategy: botasaurus` when you want to skip the default HTTP tier and force browser rendering directly
