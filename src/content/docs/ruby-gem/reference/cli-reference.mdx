...
When capture needs a fixed fetch mode to reproduce its result, the printed YAML includes a top-level `strategy:` so later `html2rss apply` uses the same mode.
