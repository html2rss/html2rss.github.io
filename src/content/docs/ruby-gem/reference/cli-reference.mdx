...
- `--strategy`: Optional request strategy (`auto`, `default`, `botasaurus`, `local_file`). Defaults to `auto`, which tries `default` -> `botasaurus`. Legacy names `faraday` and `httpx` are accepted as aliases with a deprecation warning.
