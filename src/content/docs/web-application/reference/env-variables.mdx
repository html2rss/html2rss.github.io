...
| `REQUEST_TIMEOUT_SECONDS`                | optional; total web-request timeout in seconds on the Falcon server; default `55`                                                                            |
| `WEB_CONCURRENCY`                        | optional integer `>= 1`; default `2` in production (`1` in development); number of Falcon worker processes                                                   |
| `TLS_CERTIFICATE_PATH`                   | optional; path to certificate file (e.g. `fullchain.pem`) for direct in-process Falcon TLS termination                                                       |
| `TLS_KEY_PATH`                           | optional; path to private key file (e.g. `privkey.pem`) for direct in-process Falcon TLS termination                                                         |
...
Default compose aligns **scrape total (45) ≤ feed build (50) ≤ Falcon request timeout (55)**. The **work** budget (**30s**) applies on botasaurus-scrape-api only after the browser is ready; browser boot and queue time count against the **total** scrape wall (**45s**), not against `wait_timeout_seconds`.
...
3. **html2rss client (`BOTASAURUS_SCRAPE_TIMEOUT_SECONDS`)**: **45s** HTTPX cap for POST `/scrape` (mirrors scrape total).
...
6. **html2rss-web (`REQUEST_TIMEOUT_SECONDS`)**: **55s** Falcon outer wall.
